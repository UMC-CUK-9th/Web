...
export default function MoviePage() {
  ...
  const { data: movieResponse, loading: isPending, error } = useCustomFetch<MovieResponse>(url, options);
  ...
  if (isError) {
    return (
      <div>
        <span className="text-red-500 text-2xl"> 에러가 발생했습니다: {error?.message} </span>
      </div>
    );
  }
